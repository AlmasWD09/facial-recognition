...
import { Setting_Nav_Icon } from "@/src/components/icon";
...
const SubscriptionPurchase = () => {
  ...
  const handleSubscriptionPurchase = (packageValue: string) => {
    ...
    const convertselectedPlanData = selectedPlanData
      ...
      : null;
    // console.log(convertselectedPlanData)
    if (convertselectedPlanData) {
      Cookies.remove("selectedPlan");
    }
    Cookies.set("selectedPlan", JSON.stringify(selectedPlan));
            ...
            <div className="flex flex-col md:flex-row justify-center gap-3 md:gap-8 md:px-60">
              ...
              >
                <div className="h-full w-full flex flex-col justify-between bg-white rounded-lg p-6">
                  ...
                  <div className="mt-8">
                    <Button
                      onClick={() => handleSubscriptionPurchase("events")}
                      style={{
                        background:
                          "linear-gradient(98deg, #FEAC1A 11.54%, #F84426 87.5%)",
                        padding: "10px 20px",
